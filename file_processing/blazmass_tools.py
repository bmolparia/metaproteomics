...
from metaproteomics.utils import get_lcstep, get_unmod_seq
...
def dta_select_parser(in_file, small=False, get_tax=False, check_peptides=False, get_hashes=False, 
    ...
    peptide_types_pI =  [str, str, float, float, float, float, float, float, int, float, float, float, int, str]
    peptide_types_ppm_pI =  [str, str, float, float, float, float, float, float, float, int, float, float, float, int, str]
    with open(in_file) as f:
        line = next(f)
