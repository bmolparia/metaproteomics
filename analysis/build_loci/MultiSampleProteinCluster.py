...
from metaproteomics.analysis.functional_analysis import Functionizer
from metaproteomics.analysis import taxonomy
from itertools import chain
from collections import defaultdict


def group_across_samples(protein_clusters, sample_pep_quant, db_info=None):
    ...
    for cluster_id in all_cluster_ids:
        ...
        ## this is wrong
        # need to grab the pepides quantification from the samepl pep_quant
        # because if the protein locus was filtered out in a sample, but it still has
        # one of those peptides, it won't be in here
        ...
        for sample_name in sample_names:
            pep_quant = {pep:sample_pep_quant[sample_name].get(pep,0) for pep in peptide_seq}
            cluster_peptides[sample_name] = pep_quant
        quantification = {sample_name:sum(pep_quant.values()) for sample_name,pep_quant in cluster_peptides.items()}
